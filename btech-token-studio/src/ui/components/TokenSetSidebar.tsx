...
export function TokenSetSidebar() {
  // activeTenant intentionally unused — tenant filter no longer affects which
  // sidebar entries appear (overrides aren't standalone sets). It only changes
  // the values shown in the right panel via tenant-resolver.
  const { sets, activeSetId, setActiveSet } = useTokens();
