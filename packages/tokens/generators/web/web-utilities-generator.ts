...
// =============================================================================
// Shade utilities (primitives + brand swatches)
// Generates: bg-{name}-{shade}, text-{name}-{shade}, border-{name}-{shade}
//   bg-blue-500            → background-color: var(--color-blue-500)
//   text-brand-primary-700 → color:            var(--color-brand-primary-700)
//   border-amber-300       → border-color:     var(--color-amber-300)
// =============================================================================

const SHADE_PROP_MAP: Array<{ prefix: string; prop: string }> = [
  { prefix: 'bg',     prop: 'background-color' },
  { prefix: 'text',   prop: 'color' },
  { prefix: 'border', prop: 'border-color' },
];

function buildShadeRules(
  coreColors: ResolvedTokenMap['coreColors'],
  brandSwatches: ResolvedTokenMap['brandSwatches'],
): UtilityRule[] {
  const rules: UtilityRule[] = [];

  // Primitive ramps: blue, amber, green, lightblue, neutral, …
  for (const [name, shades] of Object.entries(coreColors)) {
    for (const shade of Object.keys(shades)) {
      const cssVar = `var(--color-${name}-${shade})`;
      for (const { prefix, prop } of SHADE_PROP_MAP) {
        rules.push({
          className: `${prefix}-${name}-${shade}`,
          declarations: { [prop]: cssVar },
        });
      }
    }
  }

  // Brand swatches: brand.primary, brand.secondary
  for (const [brandName, shades] of Object.entries(brandSwatches)) {
    for (const shade of Object.keys(shades)) {
      const cssVar = `var(--color-brand-${brandName}-${shade})`;
      for (const { prefix, prop } of SHADE_PROP_MAP) {
        rules.push({
          className: `${prefix}-brand-${brandName}-${shade}`,
          declarations: { [prop]: cssVar },
        });
      }
    }
...
/*   <div data-tenant="bspace" class="bg-primary">  ← bspace colors     */
/*                                                                        */
/* Class categories:                                                      */
/*   bg-*                          background colors (semantic)           */
/*   text-*                        text colors  (also font-size scale)    */
/*   icon-*                        icon / SVG colors                      */
/*   border-*                      stroke / border colors (semantic)      */
/*   bg-{ramp}-{shade}             primitive shade — eg bg-blue-500       */
/*   text-{ramp}-{shade}           text shade       — eg text-amber-700   */
/*   border-{ramp}-{shade}         border shade     — eg border-neutral-300 */
/*   bg-brand-primary-{shade}      brand shade      — eg bg-brand-primary-50 */
/*   text-brand-secondary-{shade}                   — eg text-brand-secondary-700 */
...
export function generateUtilitiesCss(
  ...
  // ── Color — shades (primitives + brand swatches) ──────────────────────────
  const shadeRules = buildShadeRules(data.coreColors, data.brandSwatches);
  sections.push('\n/* ── Color shades — primitives + brand ───────────────────────────────── */');
  sections.push(shadeRules.map(renderRule).join('\n'));
  ...
  const total = spacingRules.length + bgRules.length + textRules.length
    + iconRules.length + borderRules.length + shadeRules.length
    + radiusRules.length + typoRules.length;
