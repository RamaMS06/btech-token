...
function dartGetFont(fam: string, entry: TypeScaleEntry, varName: string): string[] {
  ...
  if (!hasExtra) {
    ...
    if (entry.withTextColor)                  extras.push(`      color: BTechColor.text.primary,`);
...
export function generateFlutterFiles(data: ResolvedTokenMap): void {
  ...
  // font.token.dart — primitive scale tokens only.
  // BTechFontFamily and BTechFont (static accessor) live in font.theme.dart
  // to avoid duplicate class definitions in the same barrel.
  {
    const L = [HEADER, "import 'package:flutter/material.dart';\n"];
  ...
  // typography.dart barrel — includes font.theme.dart (BTechFontTheme, BTechFont static accessor).
  // font.theme.dart is generated by flutter-theme-generator into this same directory.
